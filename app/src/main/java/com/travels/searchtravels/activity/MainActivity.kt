...
//        }
        ...
        pickImageBTN.setOnClickListener {
            if (AppPreferences.isFirstLaunch(applicationContext)) {
                Toast.makeText(
                    applicationContext,
                    "Доступ Contacts необходим для работы Google Vision API",
                    Toast.LENGTH_LONG
                ).show()
            }
    ...
    private fun startStarting() {
        ...
        startingRL.animate().alpha(1f).setInterpolator(AccelerateDecelerateInterpolator())
            .setDuration(2000);
    ...
    private fun startMain() {
        ...
        contentLL.animate().alpha(1f).setInterpolator(AccelerateDecelerateInterpolator())
            .setDuration(1000);
...
//                startPicking()
                Toast.makeText(
                    this,
                    "Permission необходим для работы Google Vision API, разрешите его в настройках",
                    Toast.LENGTH_SHORT
                )
                    .show()
            }
...
//                startPicking()
            ...
            if (resultCode == Activity.RESULT_OK) {
                ...
                Toast.makeText(
                    this,
                    "Авторизацию необходима для работы Google Vision API",
                    Toast.LENGTH_SHORT
                )
                    .show()
            }
